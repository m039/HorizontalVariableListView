...
public class HorizontalVariableListView extends HorizontalListView implements OnGestureListener, FlingRunnableView {
    ...
    protected boolean overScrollingBy( int deltaX, int deltaY, int scrollX, int scrollY, int scrollRangeX, int scrollRangeY,
        ...
        onScrolling( newScrollX, right, deltaX, clampedX );
        return clampedX;
    }
    ...
    public interface OnScrollPositionChangedListener {
        void onScrollPositionChanged(HorizontalVariableListView scrollView, float position);
    }

    private OnScrollPositionChangedListener mOnScrollPositionChangedListener = null;

    public void setOnScrollPositionChangedListener(OnScrollPositionChangedListener l) {
        mOnScrollPositionChangedListener = l;
    }

    private void onScrollPositionChanged(int scrollX, int right) {
        float position = 0.0f;
        ...
        if (mMaxX == Integer.MAX_VALUE) {
            // first algorithm

            if (getChildCount() > 0 && mAdapter != null) {
                View first = getChildAt(0);

                float fwidth = first.getWidth();
                float fright = first.getRight();
                float fx = scrollX;

                float offset = 1 - (fright - fx) / fwidth;

                position = ((float) getPositionForView(first) + offset) / (float) mAdapter.getCount();
                
                // Log.d("m039", String.format("fwidth: %s, fright: %s, fx: %s",
                //                             fwidth, fright, fx));
            }
            ...
            position = (float) scrollX / (float) right;
        }

        onScrollPositionChanged(position);
    }

    protected void onScrollPositionChanged(float position) {
        if (mOnScrollPositionChangedListener != null) {
            mOnScrollPositionChangedListener.onScrollPositionChanged(this, position);
        }
    }
    ...
    public boolean onScrolling( int scrollX, int right, int deltaX, boolean clampedX ) {
        ...
        onScrollPositionChanged(scrollX, right);
